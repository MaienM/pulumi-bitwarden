...
`terraform-provider-bitwarden`.  Substitute
...
1. Create the schema by running the following command:
    ...
    ```text
    warning: resource bitwarden_something not found in provider map; skipping
    warning: resource bitwarden_something_else not found in provider map; skipping
    warning: data source bitwarden_something not found in provider map; skipping
    warning: data source bitwarden_something_else not found in provider map; skipping
    ```
...
1. **Add resource mappings:** For each resource in the provider, add an entry in
   the `Resources` property of the `tfbridge.ProviderInfo`, e.g.:
    ...
    "bitwarden_something":      {Tok: tfbridge.MakeResource(mainPkg, mainMod, "Something")},
    "bitwarden_something_else": {Tok: tfbridge.MakeResource(mainPkg, mainMod, "SomethingElse")},
    ```
...
1. **Add data source mappings:** For each data source in the provider, add an entry in the `DataSources` property of the `tfbridge.ProviderInfo`, e.g.:
    ...
    "bitwarden_something":      {Tok: tfbridge.MakeDataSource(mainPkg, mainMod, "getSomething")},
    "bitwarden_something_else": {Tok: tfbridge.MakeDataSource(mainPkg, mainMod, "getSomethingElse")},
    ```
...
1. **Add provider configuration overrides (not typically needed):** Pulumi's
   ...
   `bitwarden_API_KEY`.  Examples of common
   uses cases:
    ...
    "apikey": {
        Default: &tfbridge.DefaultInfo{
            EnvVars: []string{"bitwarden_API_KEY"},
        },
    ```
...
1. Create a new Pulumi program in the `examples/` directory, e.g.:
    ...
    npm install
    yarn link @maienm/pulumi-bitwarden
    ```
